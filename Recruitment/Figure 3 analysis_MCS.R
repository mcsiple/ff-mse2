...
# NOTE this code was originally used in Essington et al. (2015) but I am modifying/tidying/using just a subset of it.
# Most of the original code is from other people. I have used it to get kernel densities for recruitment estimates, and to compare spectral properties of simulated recruitment ts with the "real" ones (from RAM)
...
# Subset to sardines
        # toMatch = c("sard","pilchard")
        # sard.index <- grep(paste(toMatch,collapse="|"),
        #                    data$stockname, ignore.case=TRUE)
        # data<- data[sard.index,]

# Subset to anchovy/herring
        sh.toMatch = c("anchovy","herring")
        herring.anch.index <- grep(paste(sh.toMatch,collapse="|"),
                                   data$stockname, ignore.case=TRUE)
        data<- data[herring.anch.index,]
...
# Initialize Data 
data$stockname <- factor(data$stockname)
(stockid <- unique(data$stockname) )
unique(data[c("stockname", "datasource")])
...
outputR<-matrix(NA,nrow=n.years,ncol=n.stocks) # I made this Recruitment instead of growth rate (r) for my version
...
colnames(outputR)<-stockid
rownames(outputR)<-seq(min.year,max.year)  # Standardized recruitment

#####
biomass<-tapply(data$TB,data$stockname,mean,na.rm=TRUE)
...
for (i in 1:n.stocks){
  stockindex <- which(data$stockname==stockid[i])
  ...
  #ly.index<-which(year.list==last.year) #This only works if datapoints are consecutive!
  #fy.index<-which(year.list==first.year)
  
  if(n.TB>0){
    B.list<-TB.tmp/mean(TB.tmp,na.rm=TRUE)
    #  first.year.listB[i]<-first.year.TB
    outputB[paste(Year.tmp),i]<-B.list
  }
...
rownames(B.data.mat) <-rownames(R.data.mat) <-  colnames(outputB)
colnames(B.data.mat) <-colnames(R.data.mat) <-  c("Rec Length","SD.R","Beta.r","lowerCI","upperCI","beta SE","Beta.b","lowerCI","upperCI","beta SE","SD.B") # I changed the first column to be length of recruitment time series
...
#     beta.rec<-calc.beta(r)  #beta of growth rate
    rec <- ts.data
    beta.rec <- calc.beta(rec)  # beta of recruitment time series
    # save output
    R.data.mat[i,2] <- sd(rec)
    R.data.mat[i,3:6] <- beta.rec
    R.data.mat[i,7:10] <- beta.rec
    R.data.mat[i,11] <- sd(rec)
  }
}
...
estimated.beta.weights <- 1/R.data.mat[which(!is.na(R.data.mat[,3])),6] # This is   1/(SE of beta)
estimated.beta.weights <- replace(estimated.beta.weights,which(estimated.beta==0),4) #MCS : replace 0 weights with 4?
beta.kernel <- density(estimated.beta,kernel="gaussian",weights=estimated.beta.weights/sum(estimated.beta.weights))

estimated.sd.r <- ((R.data.mat[which(!is.na(R.data.mat[,2])),2])) #This is the standard deviation of the recruitment time series
...
for (sim in 1:n.sims){
  ...
  for (i in 1:n.stocks){
    ...
    while (flag==0){
      ...
      target.sd<-beta.sd[2]
      # Simulate time series and get its beta
...
###########################################################################
##############MEGSIE'S EDITS; STUFF I NEED ENDS HERE#######################
###########################################################################


# Make fried egg plot of 2D density kernel -- this is what we can sample from ---------------------------------------------
library(reshape2)
library(plyr)
#mm <- ldply(sd.beta.kernel, data.frame)
#d + stat_density2d(geom="tile", aes(fill = ..density..), contour = FALSE)
kerndata <- data.frame(beta.rec = sd.beta.kernel$x1, sd.rec = sd.beta.kernel$x2)
kerndata.long  <- data.frame(beta.rec = rep(sd.beta.kernel$x1,each=200),sd.rec = rep(sd.beta.kernel$x2,times=200))
kerndata.long$density <- NA
index.vec <- seq(1,40000,by=200)
for (x1 in 1:nrow(sd.beta.kernel$fhat)){    #nrow(sd.beta.kernel$fhat)
  kerndata.long$density[index.vec[x1]:(index.vec[x1]+199)] <- sd.beta.kernel$fhat[x1,]
}

R.data.df <- data.frame(sd.R = R.data.mat[,2],beta.R = R.data.mat[,3],names = rownames(R.data.mat))
R.data.df <- R.data.df[-which(is.na(R.data.df$sd.R)),]
R.data.df$names <- as.character(R.data.df$names)


# draw values, maybe save kernel as a file --------------------------------
# This is a big table filled with draws of beta and SD that you can use
samples <- rkernel2D(1000,sd.beta.kernel)


library(ggplot2)
ggplot(kerndata.long, aes(beta.rec, sd.rec)) + geom_tile(aes(fill=density,alpha=density)) +    #geom_point(aes(colour=density)) 
  #stat_density2d(aes(fill=..level..), size=2, bins=10, geom="polygon") + 
  scale_fill_gradient(low = "yellow", high = "red") +
  theme_classic(base_size=16) +
  geom_point(data=B.data.df,aes(x=beta.R,y=sd.R)) +
  geom_text(data=B.data.df,aes(x=beta.R,y=sd.R,label=names),nudge_y=0.05) + xlab("Recuitment beta") + ylab("Recruitment sd")
