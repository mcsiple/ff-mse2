dat <- read.csv("/Users/mcsiple/Dropbox/Chapter4-HarvestControlRules/Code/ff-mse2/Plots/DD_vs_AC_July10.csv")
remove <- c("scenario","M.type","h")
dat <- dat[,-which(colnames(dat) %in% remove)]
mdat <- melt(dat,id.vars=c("Type","obs.error.type","HCR","recruit.sd","recruit.rho"))


#dat2 <- dcast(mdat,variable+value ~ Type+obs.error.type+HCR+recruit.sd+recruit.rho)
dat2 <- dcast(mdat,Type+HCR+recruit.sd+recruit.rho+variable ~ obs.error.type)
