...
ggplot(ffdat,aes(x=Years,y=R)) + geom_line()+facet_wrap(~ASSESSID,scales="free_y")
...
for(i in 1:nstocks){
  ...
  test.constF <- calc.trajectory(lh = lh.test,obs.cv = 1.2, init = init.test, rec.dev = NA,rec.ram = rec.ram.test, F0 = F0.test, cr = cr.test, years = years.to.plot,hcr.type = "constF", const.f.rate = 0, steepness = steepness,obs.type = obs.type,equilib=equilib,R0.traj = R0.sens, tim.params = tim.params,time.var.m = NA,sig.s = 0.3,curly.phi.vec = curly.phi.mat[sim,],ac.params = ac.params)$biomass.oneplus.true
  nofish <- melt(test.constF) # unfished biomass
  nofish$stock <- stocks[i]
  nofish$year <- actual.years
  mf <- nofish
...
pdf("FIGURE_S7_B_oneplus_true_RAM_AC.pdf",width=14,height =9)
baseplot + 
