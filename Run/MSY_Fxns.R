...
# Second function determines MSY etc based on OBSERVATIONS (per reviewer 2) --------
# This makes it so that the observations are used in determining the reference points, as would occur in a real population.
# It's the same as above but just uses observation error.
getEquilibriumConditions_obs <- function(lh, fish, years, steepness){
  # Run get trajectory function across fishing vector
  # and return final biomass and surplus production
  if(is.null(steepness)){print("Steepness value is missing")}
  equilib <- sapply(fish, function(x)
