  ...
  fftype = types[f]
      ...
      #run.mod <- function(fftype){
        ...
        #toplot=FALSE      # Don't plot examples of recruitement trajectories
                ...
                source(here::here("Recruitment/GenerateDevs.R")) 
        source(here::here("Estimators/CalcFTrue.R"))
        source(here::here("Run/HCR_Trajectory_NEW.R"))
        source(here::here("Estimators/Estimators.R"))
        source(here::here("Run/generate_M.R"))
        ...
        # Sardines
        if(subDir == "Sardine"){
            source(here:here("Ctl/Sardine_LHControl.R"))
            source(here::here("Ctl/Sardine_FisheryControl.R"))
            # Sardine params
            recruit.sd <- 0.6 
            recruit.rho <- 0.9
        }
        ...
        # Anchovy/Herring
        if(subDir == "Anchovy"){
            source(here::here("Ctl/Anchovy_LHControl.R"))
            source(here::here("Ctl/Anchovy_FisheryControl.R"))
        ...
        # Menhaden
        if(subDir == "Menhaden"){
          source(here::here("Ctl/Menhaden_LHControl.R"))
          source(here::here("Ctl/Menhaden_FisheryControl.R"))
          #Menhaden recruitment dev params
          recruit.sd <- 0.8
          recruit.rho <- 0.2
        }
        
        # Create list of matrices with errors for the delayed detection scenario. This is important because the random seed will offset otherwise.
        set.seed(123)
        tim.rands.list <- list() 
        ...
        # Load harvest rules
        source(here::here("Control Rules/smith_oceana.R"))
        source(here::here("Control Rules/cfp.R"))
        source(here::here("Control Rules/hockey-stick.R"))
        source(here::here("Control Rules/trend-based-rule.R"))
        ...
        # Scenarios
        h = c(0.9, 0.6)
        obs.error.type = c("AC") #original: c("AC","Tim","noerror") 

        HCR = c("constF","C1","constF_HI") # Took out trend because it was unrealistic-- but using trend in CPUE as adjustment (data-poor method) might be a good idea!
        ...
        # Test params and runs to make sure they look good ------------------------
                ...
                ggplot(nofish,aes(x=year,y=value)) + 
                  geom_line() + 
                  facet_wrap(~L1,scales = "free_y") + 
                  geom_hline(yintercept=equilib$B0) + 
                  geom_hline(yintercept=0.5*equilib$Bmsy,col="red") 
        ...
        for(s in 1:nscenarios){  
          ...
          ac.params = list(rho=ifelse(recruit.rho==0.9,0.89,0.74), # From Wiedenmann et al. 
                           sig.s=ifelse(recruit.rho==0.9,0.34,0.37))
          ...
          for (sim in 1:nsims){
            ...
            CFP[["biomass.oneplus.obs"]][sim,] <- expt.cfp$biomass.oneplus.obs        # Observed one-plus biomass
            CFP[["biomass.total.true"]][sim,] <- expt.cfp$biomass.total.true          # True total biomass
