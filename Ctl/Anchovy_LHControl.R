...
# This file contains all the life history info used for anchovy
# These params come from multiple sources, and from different populations
# Another option would be to use params from S. Africa anchovy assessment
...
lh.test <- list(M = 1.05,   # mean between CA value (M = 1.2, from Jacobson et al. 2001, which cites Jacobson et al. (1994), Jacobson et al. (1995), and Methot (1989)), and the South Africa value (M = 0.9, Cunningham & Butterworth 2007) mean(c(1.2,0.9)
                ...
                maturity = c(0,0.4,0.85,0.99,1,1,1), # This is based on anecdotal info from Alec (see email, late January 2017).
                R0=1e9) # R0 can be changed if necessary


#  Testing FMSY and other ref points -----------------------------
# Ideally the fishery parameters (like selectivity) could be adjusted until FMSY is similar to the estimate in the stock assessment. Howevever, for these stocks they aren't manage using FMSY, so I looked for clues in the assessment about FMSY or proxy.
