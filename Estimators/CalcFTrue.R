...
# This "true F" is different than F from the HCR (the "intended F"), because the TAC is determined by "managers" in the model with the assumption that observed biomass is correct.
...
calc.true.f <- function(tac.fn = tac, M.fn = M, sel.fn = sel, Btrue.fn = NA, w.at.age = w.at.age){
        ...
        #' @param tac.fn - the TAC (calculate from B_obs and F.intended)
        ...
        fn.to.minimize <- function(param,tac = tac.fn, M = M.fn, sel = sel.fn,
          ...
          catch_age <- (Btrue * sel * F.true * (1-exp(-death_rate))) / death_rate # this is the biomass at age caught by the fishery
          pred.tac <- sum(catch_age)
          diff <- (sum(tac) - pred.tac)^2   # for optim purposes
          return(diff)
        }
