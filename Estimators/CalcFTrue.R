# This function estimates F_true, the true fishing rate based on the TAC. This is different than F from the HCR, because the TAC is determined with the assumption that the observed biomass is correct
...
calc.true.f <- function(tac.fn = tac, M.fn = M, sel.fn = sel, Btrue = NA, w.at.age = w.at.age){
        fn.to.minimize <- function(param,tac = tac.fn, M = M.fn, sel = sel.fn, Bobs_a.fn = Bobs_a,
            ...
            #' @param Btrue - vector of true biomass at age in year y
            #' 
