...
calc.true.f <- function(tac.fn = tac, M.fn = M, sel.fn = sel, Bobs_a = c(0,obs.at.age.oneplus),w.at.age =w.at.age){
        fn.to.minimize <- function(param,tac = tac.fn, M = M.fn, sel = sel.fn, Bobs_a.fn = c(0,obs.at.age.oneplus),
                                   w.at.age.fn = w.at.age) {
          F.true <- param
          death_rate <- M + sel * F.true
          catch_age <- (Bobs_a * sel * F.true * (1-exp(-death_rate))) / death_rate
